          ...
          <div className="grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-4 px-3">
            {data.map((items, index) => {
              return (
                <div
                  key={index}
                  className="flex flex-col justify-start shadow-md shadow-color-hover-bg-nav items-center border border-color-hover-bg-nav bg-color-card-bg rounded-md gap-2 relative"
                >
                  {items.url ? (
                    <a
                      href={items.url}
